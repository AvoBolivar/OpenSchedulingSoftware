...
|---|---|---|---|
| Button | `basic/button/button.tsx` | `ui/button` | `variant: "primary" \| "secondary" \| "danger"` (default `primary`) maps internally to shadcn's `default`/`secondary`/`destructive`. Still the **only** button implementation — never write a raw `<button>` styled ad hoc. |
